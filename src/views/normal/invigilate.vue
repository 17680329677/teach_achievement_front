    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible">
      <el-form :model="editForm" size="small" label-width="80px">
        ...
        <el-form-item label="申请教师工号">
          <el-input v-model="editForm.apply_teacher_number" auto-complete="off" :disabled="true"></el-input>
        </el-form-item>
        <el-form-item label="申请教师姓名">
          <el-input v-model="editForm.apply_teacher_name" auto-complete="off" :disabled="true"></el-input>
        </el-form-item>
        <el-form-item label="所属学期">
          <el-select v-model="editForm.semester_id" placeholder="请选择" class="filter-item">
            <el-option
              v-for="item in semesterInfoOptins"
              :label="item.semester_name"
              :key="item.id"
              :value="item.id"
            ></el-option>
          </el-select>
          已选择：{{ editForm.semester_name }}
        </el-form-item>
...
<script>
  ...
  } from "@/api/normal/invigilate";
  import {getSemesterInfoOptins} from "@/api/normal/optionInfo";
  ...
  export default {
    ...
    mounted: function () {
      ...
      //学期选项信息
      getSemesterInfoOptins().then(res => {
        if (res.status == 'success') {
          this.semesterInfoOptins = res.data;
        }
      })
